...
def get_quality_format(url, quality="best"):
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        # Best quality: return the last element (highest resolution)
        if quality == "best":
            return video_formats[-1]["format_id"]
        
        # Medium quality: return the next lower resolution than the best
        elif quality == "medium":
            best_resolution = video_formats[-1].get("height", 0)
            for fmt in reversed(video_formats[:-1]):  # Skip the best format
                if fmt.get("height", 0) < best_resolution:
                    return fmt["format_id"]
            return video_formats[-1]["format_id"]  # Default to best if no medium found
        
        # Low quality: return the next lower resolution than the medium
        elif quality == "low":
            best_resolution = video_formats[-1].get("height", 0)
            ...
            for fmt in reversed(video_formats[:-1]):  # Skip the best format
                if fmt.get("height", 0) < best_resolution:
                    medium_resolution = fmt.get("height", 0)
                    break
            # Now, find the low resolution (lower than medium)
            if medium_resolution is not None:
                for fmt in reversed(video_formats[:-1]):  # Skip the best format
                    if fmt.get("height", 0) < medium_resolution:
                        return fmt["format_id"]
