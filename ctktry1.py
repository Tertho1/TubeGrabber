...
class YouTubeDownloaderApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        self.url_entry = ctk.CTkEntry(self, width=500, font=("Segoe UI", 14))
        ...
        self.output_entry = ctk.CTkEntry(self, textvariable=self.output_var, width=500, font=("Segoe UI", 14))
        ...
        self.log_text = ctk.CTkTextbox(self, width=800, height=200, font=("Consolas", 12))
        self.log_text.grid(row=6, column=0, columnspan=3, padx=20, pady=10, sticky="nsew")

        # Progress Bar
        self.progress = ctk.CTkProgressBar(self, orientation="horizontal", width=800)
        self.progress.grid(row=7, column=0, columnspan=3, padx=20, pady=10, sticky="ew")
        self.progress.set(0)

        # Additional Buttons
        self.playlist_button = ctk.CTkButton(self, text="Download Playlist", command=self.download_playlist_gui)
        self.playlist_button.grid(row=8, column=0, padx=20, pady=10)

        self.playlist_audio_button = ctk.CTkButton(self, text="Download Playlist as Audio", command=self.download_playlist_audio_gui)
        self.playlist_audio_button.grid(row=8, column=1, padx=20, pady=10)

        self.convert_button = ctk.CTkButton(self, text="Convert Video to Audio", command=self.convert_video_to_audio_gui)
        self.convert_button.grid(row=8, column=2, padx=20, pady=10)
    ...
    def download_video(self, url, output_path, quality):
        try:
            ...
            if self.format_has_audio(format_id, url):
                ydl_opts = {
    ...
    def format_has_audio(self, format_id, url):
        ydl_opts = {
            "quiet": True,
        }
        with YoutubeDL(ydl_opts) as ydl:
    ...
    def download_playlist_gui(self):
        url = self.url_entry.get()
        quality = self.quality_var.get().lower()
        output_path = self.output_var.get()

        if not url:
            messagebox.showwarning("Error", "Please enter a valid YouTube playlist URL.")
            return

        # Start download in a separate thread
        download_thread = threading.Thread(
            target=self.download_playlist, args=(url, output_path, False, quality), daemon=True
        )
        download_thread.start()

    def download_playlist_audio_gui(self):
        url = self.url_entry.get()
        output_path = self.output_var.get()
        ...
        # Start download in a separate thread
        download_thread = threading.Thread(
            target=self.download_playlist, args=(url, output_path, True, "best"), daemon=True
        )
        download_thread.start()

    def download_playlist(self, url, output_path, audio_only, quality):
        try:
            ydl_opts = {
                "quiet": True,
                "extract_flat": True,
            ...
            with YoutubeDL(ydl_opts) as ydl:
                ...
                if "entries" not in info_dict:
                    self.log("This is not a playlist. Please provide a valid playlist URL.")
                    return

                video_urls = [entry["url"] for entry in info_dict["entries"]]
                self.log(f"Found {len(video_urls)} videos in the playlist.")
                ...
                for video_url in video_urls:
                    if audio_only:
                        ...
                        formats.append(self.get_quality_format(video_url, quality))

                with ThreadPoolExecutor(max_workers=5) as executor:
                    futures = []
                    for video_url, format_id in zip(video_urls, formats):
                        if audio_only:
                            futures.append(executor.submit(self.download_audio, video_url, output_path))
                        else:
                            futures.append(executor.submit(self.download_video, video_url, output_path, format_id))

                    for future in futures:
                        future.result()

                self.log(f"Playlist downloaded to: {output_path}")
        except Exception as e:
            self.log(f"Error downloading playlist: {e}")

    def convert_video_to_audio_gui(self):
        ...
        convert_thread = threading.Thread(
            target=self.convert_video_to_audio, args=(video_path, output_path), daemon=True
        )
    ...
    def convert_video_to_audio(self, video_path, output_path):
        try:
            ...
            self.log(f"Audio saved: {audio_filename}")
        except subprocess.CalledProcessError as e:
            self.log(f"Error converting video to audio: {e}")
