...
# Modify the setup_environment function to be more efficient
def setup_environment():
    """Set up the environment for both development and executable modes"""
    if getattr(sys, "frozen", False):
        ...
        # Create temporary directory - but don't do this on every startup
        ...
        # Only set up logging if needed, use append mode
        log_file = os.path.join(base_path, "tubegrabber.log")
        
        # Don't redirect stdout/stderr in normal operation - this slows things down
        # Only uncomment for debugging
        # sys.stdout = open(log_file, "a")
        # sys.stderr = sys.stdout


# Call this function before creating the Tk root
setup_environment()
...
class TubeGrabberApp:
    def __init__(self, root):
        ...
        # Set the window icon with better handling for Windows
        try:
            if getattr(sys, "frozen", False):
                # Running in a bundle
            ...
            if os.path.exists(icon_path):
                # For Windows, set both window and taskbar icons
                self.root.iconbitmap(default=icon_path)
                
                # Add this to help Windows recognize the taskbar icon
                if os.name == 'nt':
                    try:
                        import ctypes
                        myappid = 'mycompany.tubegrabber.version1'  # Arbitrary string
        ...
        self.all_search_results = []  # Store all search results for pagination
        self.current_search_query = ""  # Store the current search query
        self.active_download = False  # Flag to track if a download is in progress
        self.cancel_requested = False  # Flag to signal download cancellation
    ...
    def create_option_buttons(self):
        ...
        options = [
            ("Search Videos", "search_videos"),  # Move search option to the top
    ...
    def create_input_area(self):
        ...
        self.create_search_video_fields()  # Add search video fields
    ...
    def create_search_video_fields(self):
        ...
        # Add search type selection (Video/Playlist)
        ...
        # Create a frame inside the container to hold search results
        self.results_frame = ttk.Frame(self.results_container)
        self.results_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        # Pagination controls with improved positioning
        ...
        # Previous page button with arrow - now next to page numbers
        ...
        # Page number buttons container - now between arrows
        self.page_buttons_frame = ttk.Frame(pagination_center)
        self.page_buttons_frame.pack(side=tk.LEFT)

        # Next page button with arrow - now next to page numbers
    ...
    def create_progress_area(self):
        ...
        # Create a container for the cancel button on the right side
    ...
    def show_input_fields(self, option):
        for frame in [
            ...
            self.playlist_audio_frame,
            self.convert_video_frame,
            self.search_video_frame,  # Add search frame to hide/show
        ]:
            frame.pack_forget()
    ...
    def _download_single_video_thread(self, url, format_id):
        try:
            # Set active download flag and enable cancel button
            ...
            # Reset flags and disable cancel button
        ...
        try:
            ...
            def progress_hook(d):
                ...
                if status == "downloading" and is_main_file:
                    ...
                    # Fix for executable mode - ensure playlist index starts at 1
                    if playlist_index > 1 and current_state["current_item"] == 0:
                        # First item but index > 1, adjust to show as first item
                        display_index = 1
                    else:
                        # For subsequent items or if starting correctly at 1
                    ...
                    # Update UI - replace current_state["current_item"] with display_index
                    status_text = (
            ...
            self.download_with_retry(
                ...
                progress_hook=progress_hook,
                cancel_check=lambda: self.cancel_requested,  # Pass cancellation check
            )
    ...
    def download_video(self, url, output_path=None, format_id=None, progress_hook=None):
        try:
            ...
            # Clean up any partial files in the temp directory if it was a cancellation
            if str(e) == "Download cancelled by user":
    ...
    def search_videos(self):
        """Search for YouTube videos and playlists based on query"""
        ...
        # Calculate number of results to fetch for pagination
        # We'll fetch 100 results at once to allow for pagination
        search_count = 100

        # Get the current search type (videos or playlists)
    ...
    def _search_thread(self, query, search_count, search_type):
        """Background thread to perform YouTube search"""
        try:
            ...
            # Select appropriate search URL based on search type
            if search_type == "playlists":
                # Use specific URL parameter for playlists search
            ...
            # Use yt-dlp to search for videos and playlists
            command = [
                ytdlp_path,
                "--dump-json",
                "--flat-playlist",
                ...
                for line in result.stdout.strip().split("\n"):
                    if line.strip():
                        try:
                            ...
                            # Determine if this is a playlist based on search type and item data
                            playlist_id = None
                            if search_type == "playlists":
                                # For playlist search, check if it has playlist indicators
                                is_playlist = True  # In playlist search mode, treat all results as playlists

                                # Try to get playlist ID from various possible sources
                                ...
                                # Only add the item if we found a valid playlist ID
                                if playlist_id:
                                    # Get the thumbnail
                                    ...
                                    # Add to results directly without additional API calls
                                    self.all_search_results.append(
                                        {
                                            ...
                                            # "count": video_data.get("count", "?"),  # Use available count or "?"
                                            # "cnt": video_data.get("count", "?"),
                                            "description": (
                                ...
                                # In videos mode, identify videos
                                # Check if this has a video ID (most YouTube videos will)
                                if video_data.get(
                                    "id"
    ...
    def _display_search_results(self, results):
        """Display search results with thumbnails in the UI"""
        # Clear the results frame first
        for widget in self.results_frame.winfo_children():
            widget.destroy()
        ...
        # Configure a canvas with scrollbar for the results
        ...
        # Place canvas and scrollbar in the results frame
        ...
        # Clear thumbnail references before adding new ones
        self.thumbnail_refs = []

        # Custom styles for video/playlist items
        ...
        # Add results to the scrollable frame
        for i, item in enumerate(results):
            ...
            # Store item data for use in event handlers
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.thumbnail_refs.append(photo)  # Keep a reference
                    ...
                    # If thumbnail download fails, show placeholder
            ...
            if item["type"] == "video":
                ...
                details_text = (
                    # f"By: {item['uploader']} | Videos: {item.get('count', '?')}"
                    f"By: {item['uploader']}"
                )
            ...
            if item["type"] == "video":
                ...
                # Download playlist as audio button
                ttk.Button(
                    buttons_frame,
    ...
    def _send_to_video_download(self, url):
        """Send URL to video download section"""
    ...
    def _send_to_audio_download(self, url):
        """Send URL to audio download section"""
    ...
    def _send_to_playlist_download(self, url):
        """Send URL to playlist download section"""
    ...
    def _send_to_playlist_audio_download(self, url):
        """Send URL to playlist audio download section"""
    ...
    def show_search_page(self):
        ...
        # We'll show max 5 page numbers at a time with "..." if needed
        if total_pages <= 7:
            # If there are few pages, show all of them
            for i in range(1, total_pages + 1):
                self._create_page_button(i)
        else:
            # If there are many pages, show strategic ones with ellipsis
            if self.current_page <= 3:
                ...
                self._create_page_button(total_pages)  # Last page
            elif self.current_page >= total_pages - 2:
                # Near the end
                self._create_page_button(1)  # First page
                ttk.Label(self.page_buttons_frame, text="...").pack(
                    side=tk.LEFT, padx=2
                )
    ...
    def show_next_page(self):
        ...
        # Only proceed if there's a next page
    ...
    def show_previous_page(self):
        ...
        # Only proceed if there's a previous page
    ...
    def cancel_download(self):
        ...
        if self.active_download:
            ...
            # The cancel_button will be disabled once the download completes
            # in the finally block of the download thread methods

    def _cleanup_temp_files(self):
        """Clean up all temporary files in the temp directory that might be left from cancelled downloads"""
        temp_dir = self.get_temp_dir()
        try:
            if os.path.exists(temp_dir):
                # Wait briefly to allow processes to release files
                time.sleep(0.5)
                ...
                for filename in os.listdir(temp_dir):
                    ...
                    if os.path.isfile(file_path):
                        try:
                            # Try deleting the file a few times with delays
                            max_attempts = 3
                            for attempt in range(max_attempts):
                                try:
                                    ...
                                    # If file is still in use, wait a bit and retry
                                    if attempt < max_attempts - 1:
                                        time.sleep(1)  # Increase delay between attempts
                                    else:
                                        print(
                                            f"Could not delete file after {max_attempts} attempts: {file_path}"
                                        )
                                        # Log the skipped file but don't raise an exception
                        except Exception as e:
                            # Just log other errors but continue with other files
...
def download_playlist(
    ...
    audio_only=False,
    progress_hook=None,
    cancel_check=None,  # Add a callback to check for cancellation
):
    try:
        temp_dir = TEMP_DIR
        ydl_opts = {
        ...
        # Custom downloader to check for cancellation between videos
        class CancellableYoutubeDL(YoutubeDL):
            ...
            def extract_info(
                ...
                # Check for cancellation before starting each video extraction
