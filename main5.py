...
def list_and_choose_format(url, format_type="video"):
    ...
    with YoutubeDL(ydl_opts) as ydl:
        ...
        for i, fmt in enumerate(formats):
            print(
                f"{i + 1}. {fmt['format']} - {fmt.get('resolution', 'N/A')} - {fmt.get('ext', 'N/A')}"
            )
        ...
        try:
            ...
            if 0 <= choice < len(formats):
                ...
                print(
                    f"Invalid choice. Downloading the best {format_type} format by default."
                )
                return "best"
        except ValueError:
            print(
                f"Invalid input. Downloading the best {format_type} format by default."
            )
            return "best"
        ...
        if format_id is None:
            ...
            with YoutubeDL(ydl_opts) as ydl:
                ...
                if audio_formats:
                    ...
                    print(
                        "No audio formats found. Downloading the best available format."
                    )
                    format_id = "best"
...
#                     else:
#                         # Let the user choose the format for each video
...
# Function to choose a format based on quality preference
def choose_format_by_quality(url, quality="best"):
    ydl_opts = {"quiet": True}  # Suppress yt-dlp output
    with YoutubeDL(ydl_opts) as ydl:
        info_dict = ydl.extract_info(url, download=False)
        formats = info_dict.get("formats", [])
        ...
        video_formats.sort(key=lambda x: int(x.get("height", 0)))

        # Select format based on quality level
        if quality == "low":
            return (
                video_formats[-3]["format_id"]
                if len(video_formats) >= 3
                else video_formats[0]["format_id"]
            )
        elif quality == "medium":
            return (
                video_formats[-2]["format_id"]
...
# Updated playlist downloader with quality selection
def download_playlist(url, output_path="downloads", audio_only=False):
    try:
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if not audio_only:
                ...
                quality_map = {"1": "low", "2": "medium", "3": "best"}
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                ...
                for video_url in video_urls:
                    if audio_only:
                        futures.append(
                            executor.submit(download_audio, video_url, output_path)
                        )
                    else:
                        format_id = choose_format_by_quality(video_url, quality)
                        futures.append(
                            executor.submit(
...
def convert_video_to_audio(video_path, output_path="downloads"):
    try:
        ...
        command = [
            ...
            video_path,  # Input video file
            "-q:a",
            "0",  # Best audio quality (variable bitrate)
            "-map",
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            if filename:
                convert = input(
                    "Do you want to convert this video to audio? (y/n): "
                ).lower()
                if convert == "y":
